...
const copy = {
  components: {
    ...
    headers: ['Component', 'Category', 'Status', 'ID']
  },
  blocks: {
    title: 'Block Catalog',
...
function render(){
  ...
  if(cards){
    cards.innerHTML = rows.slice(0, 12).map(row => {
      ...
      return `<article class="library-item"><div class="card-top"><span class="sg-meta">${row[0]}</span>${badge(status)}</div><h3>${row[1]}</h3><p>${category}</p>${href !== '#' ? `<a class="sg-button small primary" href="${href}">Open Preview</a>` : '<span class="sg-meta">DOC PAGE PLANNED</span>'}</article>`;
    }).join('');
  }
  if(table){
